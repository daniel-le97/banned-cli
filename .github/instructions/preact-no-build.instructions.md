...
# Preact No-Build Setup Guidelines

This project uses Preact with no build step for frontend development. Follow these patterns for consistency and best practices.
...
While not an exhaustive list, here are some common patterns and recipes you may find useful when working with import maps.
...
1. **Use import maps** for module resolution instead of full URLs in JavaScript files
2. **Always use `?external=preact`** when importing Preact-dependent modules from esm.sh
3. **Prefer HTM syntax** over JSX for template literals (no build step required)
4. **Keep components in separate files** with `.js` extensions for better organization
5. **Use semantic HTML** and accessible patterns in component templates
6. **Follow modern ES modules** patterns with explicit imports/exports
